...
ggplot(PR_PEVASPEA_SINASC_NASC_SE_GERAL, 
       aes(x = Tempo, 
           y = Casos)) +
...
cores <- c("Casos Brutos" = "grey70", 
           ...
           "Tendência (LOESS)" = "darkred")

Teste <- ggplot(PR_PEVASPEA_SINASC_ANOMAL_SE_GERAL, aes(x = Tempo)) +
  annotate("rect",
           xmin = 209, 
           xmax = 313, 
           ymin = -Inf, 
  ...
  labs(
    x = "Ano",
    y = "Nascimentos com Anomalias",
    subtitle = "A linha escura representa a média móvel (tendência); a área em destaque compreende o período da pandemia."
  )
...
############  Taxa 04 anos municípios estado

AUX <- PR_PEVASPEA_SINASC_Serie_historica_Mun[, 1]

AUX <- PR_PEVASPEA_SINASC_Serie_historica_Mun %>%
  mutate(TAXA_5a = ((PR_PEVASPEA_SINASC_Serie_historica_Mun$N_2025 + 
                      ...
                      1000), 
         Nascidos_5a = (PR_PEVASPEA_SINASC_Serie_historica_Mun$Nascidos_2025 + 
                          PR_PEVASPEA_SINASC_Serie_historica_Mun$Nascidos_2024 +
                          PR_PEVASPEA_SINASC_Serie_historica_Mun$Nascidos_2023 +
                          PR_PEVASPEA_SINASC_Serie_historica_Mun$Nascidos_2022 ),
         Anomalias_5a = (PR_PEVASPEA_SINASC_Serie_historica_Mun$N_2025 + 
                           PR_PEVASPEA_SINASC_Serie_historica_Mun$N_2024 +
                           PR_PEVASPEA_SINASC_Serie_historica_Mun$N_2023 +
                           PR_PEVASPEA_SINASC_Serie_historica_Mun$N_2022)
  )
...
#############   Taxa Anomalias/1000 nascimentos nos últimos 06 anos somados
#############   por Regionais de Saúde

AUX <- PR_PEVASPEA_SINASC_RS_Serie_Historica[, 1]

AUX <- PR_PEVASPEA_SINASC_RS_Serie_Historica %>%
  mutate(TAXA_5a = ((PR_PEVASPEA_SINASC_RS_Serie_Historica$Anomalias_2025 + 
...
PR_SINASC_MAP_TAXA_5_ANOS_ANOMAL_RS <- ggplot() +
...
ggsave("/home/gustavo/Área de trabalho/Análise_de_Dados/Imagens/SINASC/PR_SINASC_MAP_TAXA_4_ANOS_ANOMAL_RS.png",
       PR_SINASC_MAP_TAXA_5_ANOS_ANOMAL_RS,
       width = 56,
       height = 46,
       units = "cm",)

ggsave("/home/gustavo/Área de trabalho/Análise_de_Dados/Imagens/SINASC/Teste.png",
       Teste,
       width = 25,    
       height = 10,     
       dpi = 300,
...
### Realizando a suavização dos dados com Método Bayesiano Empírico

Taxa_Suavizada <- EBlocal(ri = MAPA_BASE_PR$Anomalias_5a, 
                          ni = MAPA_BASE_PR$Nascidos_5a,
